...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        if (userEmail != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            AppUser user = appUserRepository.findByEmailAndIsDeletedFalse(userEmail).orElse(null);

            if (user != null && jwtService.isTokenValid(jwt, user.getEmail())) {
                // Invalidate tokens issued before password was last changed
                java.util.Date issuedAt = jwtService.extractIssuedAt(jwt);
                if (user.getPasswordChangedAt() != null && 
                    issuedAt.before(java.sql.Timestamp.valueOf(user.getPasswordChangedAt()))) {
                    filterChain.doFilter(request, response);
                    return;
                }
